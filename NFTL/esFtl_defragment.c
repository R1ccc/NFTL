...
int esFtl_MarkedFirstBlock(int bno)
{
    ...
    uint32_t page = bno % ESFTL_NANDNUMBLOCKS;
    //bno = bno % ESFTL_NANDNUMBLOCKS;
    
    if(SPI_NAND_FAIL == nandflash_page_program(&markedByte, bno, 0, 1)){
                printf("\n\rWrite page failed!\n\r");
                while(1);
    }
    // rv = esFtl_NandFlashWrite(page * ESFTL_NANDNUMPAGEBLOCK, ESFTL_NANDPAGEDATASIZE + 5, &markedByte, 1);
    // if (rv)
...
void esFtl_Defrag(void)
{
    ...
    if (endBlock != startBlock)
    {
        ...
        do
        {
            ...
            for (j = 0; j < ESFTL_NANDNUMPAGEBLOCK; j++)
            {
